...
#sol.append(Relief(data, target))
#sol.append(PCC(data, target))
#sol.append(f_regression_score(data, target))
#sol.append(mutual_info_regression_score(data, target))
#sol.append(MAD(data, target))
#sol.append(Dispersion_ratio(data, target))
#sol.append(feature_selection_sim(data, target))
sol.append(Fisher_score(data, target))
...
while gen_no < max_gen:
    ...
    function1_values2 = function1(solution2, X_train, y_train, X_test, y_test).tolist()
...
for f in feat_unique:
    ...
    for i in range(df.shape[0]):
        if df[i, 1] == f:
            mse_li.append(df[i, 0])
    min_mse = min(mse_li)
    if min_mse < thresh:
        kk = np.expand_dims(np.asarray([min_mse, f]), axis=0)
