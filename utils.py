...
import sklearn.svm
import random
from sklearn.metrics import classification_report, balanced_accuracy_score,confusion_matrix, mean_squared_error, r2_score
...
def cal_pop_fitness(pop, train_datas, train_labels, test_datas, test_labels):
    pop = np.array(pop)
    mses = np.zeros(pop.shape[0])
    r2s = np.zeros(pop.shape[0])
    idx = 0
    ...
    for curr_solution in pop:
        ...
        ## SVM REGRESSOR ##
        SVM_regressor = sklearn.svm.SVR(kernel='rbf', gamma='scale', C=5000)
        SVM_regressor.fit(X, y)
        predictions = SVM_regressor.predict(reduced_test_features)

        mse, r2 = regression_metrics(test_labels, predictions)
        mses[idx] = mse
        r2s[idx] = r2
        
        idx = idx + 1
    return mses, r2s, predictions
